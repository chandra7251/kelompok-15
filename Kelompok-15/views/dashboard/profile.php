            ...
            <div class="flex flex-col sm:flex-row items-center sm:items-center mt-6 mb-12 sm:space-x-4 -ml-4">
                ...
                <div class="flex flex-col items-center gap-3 relative z-10 flex-shrink-0">
                    <div class="relative group">
                        <div class="w-32 h-32 rounded-full border-4 border-[#0F2942] overflow-hidden bg-white shadow-lg flex items-center justify-center transition-transform group-hover:scale-105">
                            ...
                            <label for="photoInput" class="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer transition-all duration-300">
                            ...
                            <form id="photoForm" action="index.php?page=profile&action=update_photo" method="POST" enctype="multipart/form-data" class="hidden">
                                <?= csrf_field() ?>
                    ...
                    <?php if (isset(auth()['photo']) && !empty(auth()['photo'])): ?>
                        <form action="index.php?page=profile&action=delete_photo" method="POST" onsubmit="return confirm('Hapus foto profil?')">
                            <?= csrf_field() ?>
                            <button type="submit" class="text-rose-400 hover:text-rose-300 text-xs font-bold uppercase tracking-wider flex items-center gap-1 transition-colors">
