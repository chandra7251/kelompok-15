...
<div class="max-w-7xl mx-auto px-4 sm:px-8 py-6 md:py-10 space-y-6 md:space-y-8">
    ...
    <div class="bg-[#0F2942] rounded-2xl p-6 md:p-8 border border-white/5 shadow-xl relative overflow-hidden">
        <div class="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8 relative z-10">
            ...
            <!-- Rasio Box -->
            <div class="bg-[#051933] px-6 md:px-8 py-4 md:py-5 rounded-2xl border border-white/5 shadow-inner w-full md:w-auto md:min-w-[200px] text-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        <!-- Pemasukan -->
        <div class="bg-gradient-to-br from-emerald-900/50 to-emerald-800/20 rounded-2xl p-5 md:p-6 border border-emerald-500/20 shadow-lg relative overflow-hidden group">
            ...
            <div class="flex items-center gap-4 mb-4 relative z-10">
                 <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-emerald-500/20 flex items-center justify-center border border-emerald-500/30">
                    <svg class="w-5 h-5 md:w-6 md:h-6 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 11l5-5m0 0l5 5m-5-5v12"/></svg>
                 </div>
                 <div>
                     <p class="text-emerald-100/70 text-xs md:text-sm font-medium">Pemasukan Bulan Ini</p>
                     <p class="text-xl md:text-2xl font-bold text-white mt-1"><?= format_rupiah($spendingStatus['pemasukan']) ?></p>
                 </div>
            </div>
        </div>

        <!-- Pengeluaran -->
        <div class="bg-gradient-to-br from-rose-900/50 to-rose-800/20 rounded-2xl p-5 md:p-6 border border-rose-500/20 shadow-lg relative overflow-hidden group">
            ...
            <div class="flex items-center gap-4 mb-4 relative z-10">
                 <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-rose-500/20 flex items-center justify-center border border-rose-500/30">
                    <svg class="w-5 h-5 md:w-6 md:h-6 text-rose-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 13l-5 5m0 0l-5-5m5 5V6"/></svg>
                 </div>
                 <div>
                     <p class="text-rose-100/70 text-xs md:text-sm font-medium">Pengeluaran Bulan Ini</p>
                     <p class="text-xl md:text-2xl font-bold text-white mt-1"><?= format_rupiah($spendingStatus['pengeluaran']) ?></p>
                 </div>
            </div>
        </div>

        <!-- Selisih / Net Balance -->
        <div class="bg-gradient-to-br from-indigo-900/50 to-indigo-800/20 rounded-2xl p-5 md:p-6 border border-indigo-500/20 shadow-lg relative overflow-hidden group">
            ...
            <div class="flex items-center gap-4 mb-4 relative z-10">
                 <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-indigo-500/20 flex items-center justify-center border border-indigo-500/30">
                 ...
                 <div>
                     ...
                     <p class="text-xl md:text-2xl font-bold text-white mt-1"><?= format_rupiah($spendingStatus['selisih']) ?></p>
                 </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid lg:grid-cols-12 gap-6 md:gap-8">
        ...
        <div class="lg:col-span-8 space-y-6 md:space-y-8">
            ...
            <div class="bg-[#0F2942] rounded-2xl shadow-xl border border-white/5 overflow-hidden">
                <div class="p-4 md:p-6 border-b border-white/5 flex justify-between items-center">
                    ...
                    <span class="text-[10px] md:text-xs text-[#B3C9D8] bg-white/5 px-2 md:px-3 py-1 rounded-full">per Kategori</span>
                </div>
                        ...
                        <tbody class="divide-y divide-white/5">
                            <?php foreach ($detailedCalc['categories'] as $cat): ?>
                                <tr class="hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td class="py-3 md:py-4 px-4 md:px-6 text-right text-rose-400 font-medium"><?= format_rupiah($cat['jumlah']) ?></td>
                                    <td class="py-3 md:py-4 px-4 md:px-6 text-right text-[#B3C9D8]"><?= $cat['persentase'] ?>%</td>
                            ...
                            <tr class="bg-indigo-500/5">
                                <td class="py-3 md:py-4 px-4 md:px-6 font-medium text-indigo-300">💰 Tabungan (Bonus)</td>
                                <td class="py-3 md:py-4 px-4 md:px-6 text-right text-green-400 font-bold"><?= format_rupiah($detailedCalc['tabungan']) ?></td>
                                <td class="py-3 md:py-4 px-4 md:px-6 text-right text-indigo-300"><?= $detailedCalc['tabungan_persentase'] ?>%</td>
                                <td class="py-3 md:py-4 px-4 md:px-6 text-center">
                                    <span class="px-2 md:px-2.5 py-0.5 md:py-1 rounded-lg text-[10px] md:text-xs font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">
        ...
        <!-- Right Side Widgets (Score & Distribution) -->
        <div class="lg:col-span-4 space-y-6 md:space-y-8">
            <!-- Financial Score Card -->
             <div class="bg-gradient-to-br from-[#0A2238] to-[#0F2942] rounded-2xl p-6 border border-white/5 shadow-xl text-center relative overflow-hidden">
                <div class="relative z-10">
                    <p class="text-[#B3C9D8] text-xs md:text-sm font-medium mb-2 uppercase tracking-wider">Skor Kesehatan</p>
                    <div class="text-5xl md:text-6xl font-[900] bg-gradient-to-r from-[#00C6FB] to-[#00F29C] bg-clip-text text-transparent mb-2">
                        <?= $detailedCalc['skor_komposit'] ?>
                    </div>
                    <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 mb-4">
                        ...
                        <span class="text-[10px] md:text-xs font-bold text-white uppercase"><?= e($detailedCalc['status_akhir']) ?></span>
                    </div>
            ...
            <div class="bg-[#0F2942] rounded-2xl p-4 md:p-6 border border-white/5 shadow-xl">
                <h3 class="font-bold text-base md:text-lg text-white mb-4 md:mb-6 text-center">Distribusi Pengeluaran</h3>
             ...
             <!-- Total Savings -->
            <div class="bg-gradient-to-br from-purple-900/60 to-indigo-900/60 rounded-2xl p-6 border border-purple-500/20 shadow-xl">
