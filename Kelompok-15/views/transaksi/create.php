...
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        <a href="index.php?page=transaksi" class="text-indigo-600 hover:text-indigo-700 text-sm font-medium">←
            Kembali</a>
        <h1 class="text-2xl font-bold text-gray-800 mt-2">Tambah Transaksi</h1>
    </div>
        ...
        <form action="index.php?page=transaksi&action=store" method="POST" class="space-y-6">
                    ...
                    <optgroup label="📥 Pemasukan">
                        <?php foreach ($kategoris as $kat): ?>
                            <?php if ($kat['tipe'] === 'pemasukan'): ?>
                                <option value="<?= $kat['id'] ?>"><?= e($kat['nama']) ?></option>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </optgroup>
                ...
                <div>
                    ...
                    <select name="mata_uang"
                        ...
                        <?php foreach ($currencies as $code => $name): ?>
                            <option value="<?= $code ?>" <?= $code === 'IDR' ? 'selected' : '' ?>><?= $code ?> - <?= $name ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3"
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent"><?= old('keterangan') ?></textarea>
            </div>
