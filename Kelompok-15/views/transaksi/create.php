...
<div class="max-w-xl mx-auto px-4 sm:px-0 py-6 sm:py-8">
    <div class="mb-6 sm:mb-8 relative">
        <a href="/transaksi" class="text-[#00C6FB] hover:text-[#00F29C] text-sm font-medium transition-colors">←
            Kembali</a>
        <h1 class="text-xl sm:text-2xl font-bold text-[#00C6FB] mt-4 text-center">Tambah Transaksi</h1>
    ...
    <div class="bg-[#0F2942] rounded-2xl p-5 sm:p-8 shadow-xl border border-white/5 relative overflow-hidden">
        <form action="/transaksi/store" method="POST" class="space-y-5">
            <?= csrf_field() ?>
                    ...
                    <optgroup label="📥 Pemasukan" class="text-[#00F29C] bg-[#0A2238]">
                        <?php foreach ($kategoris as $kat): ?>
                            <?php if ($kat['tipe'] === 'pemasukan'): ?>
                                <option value="<?= $kat['id'] ?>" class="text-white bg-[#0A2238]"><?= e($kat['nama']) ?>
                                </option>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </optgroup>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="number" name="jumlah" required min="1" step="0.01" value="<?= old('jumlah') ?>"
                        placeholder="0"
                        class="w-full px-3 py-3 bg-[#0A2238] border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#00C6FB] focus:border-transparent outline-none placeholder-gray-600">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-2">Mata Uang</label>
                    <select name="mata_uang"
                        class="w-full px-3 py-3 bg-[#0A2238] border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#00C6FB] focus:border-transparent outline-none">
                        <?php foreach ($currencies as $code => $name): ?>
                            <option value="<?= $code ?>" class="bg-[#0A2238]" <?= $code === 'IDR' ? 'selected' : '' ?>>
                                <?= $code ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div>
                ...
                <input type="date" name="tanggal" required value="<?= old('tanggal', date('Y-m-d')) ?>"
                    class="w-full max-w-[200px] px-3 py-3 bg-[#0A2238] border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#00C6FB] focus:border-transparent outline-none [color-scheme:dark] text-sm">
            </div>

            <!-- Keterangan -->
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Keterangan <span
                        class="text-gray-500">(opsional)</span></label>
                <textarea name="keterangan" rows="2" placeholder="Deskripsi singkat..."
                    class="w-full px-4 py-3 bg-[#0A2238] border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#00C6FB] focus:border-transparent outline-none placeholder-gray-600 resize-none"><?= old('keterangan') ?></textarea>
            </div>
            ...
            <div class="flex gap-3 pt-4">
                ...
                <a href="/transaksi"
                    class="px-6 py-3 border border-white/20 rounded-xl font-medium text-white hover:bg-white/5 transition text-center">
                    Batal
                </a>
            </div>
        </form>
