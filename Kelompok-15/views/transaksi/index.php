...
<div class="w-full mx-auto px-3 md:px-6 py-4 pb-20 space-y-6 animate-[fadeIn_0.5s_ease-out]">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-[#00C6FB]">Transaksi</h1>
            <p class="text-gray-400 mt-1">Kelola Pemasukan dan Pengeluaran</p>
        </div>
        <a href="index.php?page=transaksi&action=create"
            class="bg-[#00C6FB] text-[#051933] px-6 py-2.5 rounded-xl font-bold hover:bg-[#00F29C] transition shadow-[0_0_15px_rgba(0,198,251,0.3)] flex items-center gap-2">
    ...
    <div class="bg-[#0F2942] rounded-2xl shadow-xl border border-white/5 overflow-hidden">
        <?php if (empty($transaksis)): ?>
            <div class="text-center py-16">
                <div class="w-20 h-20 bg-[#0A2238] rounded-full flex items-center justify-center mx-auto mb-4 border border-white/5">
                    <span class="text-4xl">📋</span>
                </div>
                <p class="text-gray-400 mb-4">Belum ada transaksi</p>
                <a href="index.php?page=transaksi&action=create" class="text-[#00C6FB] font-medium hover:text-[#00F29C]">Tambah transaksi pertama
                    →</a>
            </div>
        <?php else: ?>
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-[#0A2238]/50 border-b border-white/5">
                        <tr>
                            <th class="text-left py-4 px-6 text-lg font-bold text-white">Tanggal</th>
                            <th class="text-left py-4 px-6 text-lg font-bold text-white">Kategori</th>
                            <th class="text-left py-4 px-6 text-lg font-bold text-white">Keterangan</th>
                            <th class="text-right py-4 px-6 text-lg font-bold text-white">Jumlah</th>
                            <th class="text-center py-4 px-6 text-lg font-bold text-white">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-white/5">
                        <?php foreach ($transaksis as $trx): ?>
                            <tr class="hover:bg-[#0A2238] transition border-b border-white/5">
                                ...
                                <td class="py-4 px-6">
                                    <span
                                        class="inline-flex items-center px-4 py-1.5 rounded-full text-xs font-medium border
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex items-center justify-center space-x-3">
                                        ...
                                        <form action="index.php?page=transaksi&action=delete" method="POST" class="inline"
                                            ...
                                            <button type="submit"
                                                class="p-2 text-white hover:text-[#FF6B6B] transition">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
